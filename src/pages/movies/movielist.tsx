    ...
    const fetchMovies = async () => {
      try {
        ...
        const response = await axios.get<{ movies: MovieDetails[] }>(
          "https://young-fjord-14925-178a5007d1a8.herokuapp.com/api/movies"
        );
  ...
  const deleteMovie = async (id: number) => {
    try {
      ...
      if (isLoggedIn() && user?.isAdmin) {
        ...
        await axios.delete(
          `https://young-fjord-14925-178a5007d1a8.herokuapp.com/api/delete_movie/${id}`
        );
      ...
      <div>
        ...
        <div className="react-skeletons">
          {filteredMovies?.slice(0, visibleMovies).map((movie) => (
            <div key={movie.id} className="skeleton">
              <div
